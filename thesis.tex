...
The aim of the first part has not been to \emph{introduce} density functional theory,\footnote{
...
but rather to \emph{describe} or summarise important concepts in the theory (sec. \ref{sec:materials}--\ref{sec:kohnsham}) as implemented in \vasp\ (sec. \ref{sec:xc}--\ref{sec:pseudopot}) in a somewhat superficial way, 
and since notation varies between different authors, to provide a consistent notation to be used when referring to the concepts later on. 
...
LDA is the best ''ab initio'' local approximation,\comment{(Korrekt? Trenger sitering?)} so to improve on LDA the logical next step is to move to a gradient approximation, including some dependency on the density gradient $\nabla\rho$, but exactly how to do this is far from straightforward and different and many different functionals have evolved over the years. 
...
\section{Bloch waves and the plane wave basis}\label{sec:planewavebasis}
...
\subsection{Forces from the Hellman-Feynman force theorem}\label{sec:forcetheorem}
...
The lattice parameter fulfilling such conditions is indicated by a veritcal line.
...
As we see from figure \ref{fig:si_bulk_conv_vol}, the most important factor is the choice of the potential, or more specifically the choice of the exchange-correlation functional.
...
%Note that a separate series was carried out using the standard 
%Note that the ``very soft'' potential used in this work (and introduced in section \ref{sec:pseudopot}) was compared with the standard potential, but no difference
