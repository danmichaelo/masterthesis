...
\subsection{Short-scale order in the melt}
...
\begin{equation}
  ...
  \label{eq:gofr}
\end{equation}
For any given atom, the number of atoms $N(r,r+\Delta r)$ within a shell of thickness $\Delta r$ can be counted, and average the number over the timespan of a simulation. 
The thickness of the shell must be chosen thin enough to capture the structure of the function, while thick enough to include enough counts, typically in the order of 0.1~Å.\cite[263]{Haile:1992}
With the volume of the shell being $V(r,\Delta r) = \frac{4}{3}\pi [(r+\Delta r)^3 - r^3)]$, we can write
\begin{equation}
    g(r) = \frac{\langle N(r,r+\Delta r) \rangle}{\rho V(r,\Delta r)}
  \label{eq:gofr}
\end{equation}
The function falls to zero for $r\to 0$ due to repulsive forces between atoms, and approaches unity for $r\to\infty$ since there is no long range order in liquids.
...
The data available to compute the P-Si pair function is of course only 1/64 the data available to compute the Si-Si pair function, which is why the P-Si function is more jagged. 
...
\begin{equation}
    ...
    \label{eq:coordno}
\end{equation}
where the integration is carried out until the first minimum to find the ``nearest neighbours''. 
The process is illustrated in fig. \ref{fig:rdffirstcoord}.
As expected, this yields a coordination number of approximately four when evaluated at the deep minimum found for a system in the crystalline state.
...
In fig. \ref{fig:rdffirstcoord}, it was evaluated to the first minimum in $g(r)$, for which $n(3.18) = 6.1$. 
Evaluated to the first minimum in $g(r)r^2$ instead, we obtain instead $n(3.04)=5.5$.
Instead of the abrupt cutoff, other methods have been, such as evaluating the peak as symmetric, integrating to the top of the peak only, and multiplying the value by two, (VAR DET i Alder:1968?)
but I'm not sure if there is really a physical motivation for such an approach.

An interesting aspect of molecular dynamics is that it also provides the dynamic data underlying the static pair distribution function. 
In fig. \ref{fig:nn_melting}, the distance between a silicon atom and its 16 nearest neighbours (4 in first `shell', 12 in `second') is plotted for each of the 64 silicon atoms, with each distance plotted as a small point.
...
The disambuigity in defining the integration limit in \eqref{eq:coordno} is perhaps even clearer here. 
