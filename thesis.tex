...
%And the PhD thesis of (Bentzen:2006) provides an up-to-date account of the diffusion behaviour of phosphorus in the specific case of the very highly doped emitter layer.
...
Diffusion\index{diffusion} is macroscopic redistribution of atoms\footnote{I will only be concerned with atoms here, but the concept of diffusion also applies to other small particles such as molecules, viruses and bacteria.} due to their microscopic thermal motion. 
...
%\footnote{\comment{Clarify macroscopic / microscopic?}}
...
%For solids, and especially crystals, the dynamic atomic motions is often described in terms of discrete jumps, a concept that will now be introduced, since it will be referred to later on.
%An approach often taken in other theoretical models than molecular dynamics, when studying solids
%the dynamic atomic motion is therefore replaced by discrete jumps, a concept that will now be discussed, since it will be referred to later on, and since it's so prevalent.
%A short discussion of other approaches is therefore justified, especially since these to a large degree have formed the way we understand and describe diffusion.
...
%Estimated jump frequency using \eqref{eq:jumpfreq-est} is plotted 
...
%Diffusion coefficients in liquids are typically of the order of $10^{-7}$ cm2/s,
%while in solids their magnitude are much smaller, typically of the order of $10^{-11}$ cm2/s. \comment{[kilder!]}
...
while the \spl{110} site has $C_{2v}$ symmetry for equivalent atoms, reducing to $C_{1h}$ (just a mirror plane) for inequivalent atoms.\cite[12]{Pichler:2004}
...
%\footnote{/Users/danmichael/Documents/Studier/Master/notur/hipersol/templates/si64/movie2.avi }
...
A 2006 NEB study by Harrison \text{et al.} confirmed a hexagonal site as the most stable one for the +1 charge state, 
...
%The mobile state can be either a pure interstitial P$_{\text{i}}$, or an interstitialcy -- a phosphorus-silicon pair (PI) -- sharing a site. 
%This process of creating a mobile defect is often called a ``kick out'' reaction;\footnote{
%    In the first reaction, the silicon self-interstitial (I) becomes a lattice atom.
%    It may, at last to me it does, seem like it disappears, but the reason for it's removal from the equation is that it's no longer a defect.
%    For an introduction to defect nomenclature, see e.g. \cite{Fahey:1989}. 
%    %I'm using upright letters according to ISO regulations.\cite{Beccari:1997}
%}
%Correspondingly, the diffusion mechanism may be referred to as either the interstitial mechanism or the interstitialcy mechanism.
%The interstitialcy mechanism has also been referred to as the `dumbbell mechanism' when the PI pair forms a dumbbell configuration.\cite{Posselt:2008} 
% interstitialcy introduced by Seitz, F. On the theory of diffusion in metals. Acta Cryst. 3, 355–363 (1950).
...
\begin{align}
    ...
    %&= H_{\text{PI}}^{\text{m}} + H_{\text{I}}^{\text{f}} - E_{\text{PI}}^{\text{b}}
\end{align}
...
Liu \textit{et al}. (2003) found a migration energy of 0.6--0.7 eV for a mechanism where P migrates between \spl{110} sites through hexagonal sites.\cite{Liu:2003}
...
%With migration barriers of 0.18 or 0.6--0.7 eV,  should be within reach of sit \textit{ab initio} molecular dynamics.
...
%Despite a similar diffusion mechanism mediated by Si self-interstitials, P exhibits a lower activation energy than B because of stronger bonding to the Si self-interstitial.

%Diffusion of substitutional impurities as interstitials or impurity-self-interstitial pairs.

%The intersititialcy mechanism involves a silicon self-interstitial (I) meeting 
%a phosphorus atom (P), making a mobile phosphorus-silicon self-interstitial (PI) complex, 
%that diffuses for some (short) distance, possibly swapping the silicon atom, before phosphorus 
%returns to a lattice site.
...
%where $H_{\text{PI}}^{\text{m}}$ is the migration enthalpy of the PI complex, 
%%$H_{\text{PI}}^{\text{f}}$ the formation enthalpy of the complex -- 
%%$H_{\text{I}}^{\text{f}}$ the formation enthalpy of the self-interstitial
%%and $E_{\text{PI}}^{\text{b}}$ the binding energy of the PI complex.\cite[323]{Fahey:1989}
%With our initial configuration, we are effectively starting with a PI complex, 
%meaning we have already 

%we may be able to calculate the migration enthalpy of the PI complex,

%that is the activation energy of diffusion minus the creation of the phosphorus interstitial.
%The phosphorus interstitial can be created from a collision with a silicon self interstitial, 
%but the activation energy for this process is expected to be too large to simulate at the \textit{ab initio} level.


%While the exact choice of interstitial configuration is not expected to have 
...
%\comment{inkluderer dette $T-T_0$ og selv-interaksjonskorreksjon?}
}
...
\begin{equation}
  ...
  \label{eq:pwbasis}
\end{equation}
%Refer to appendix \ref{spp:pwbasis} for more details.
...
%Pulay\cite{Pulay:1969} has generalised the theorem by adding terms known as Pulay forces to account
...
\footnote{
    ...
    %The Hamiltonian here is thus a function of $6N$ coordinates: $3N$ positions $r = \{\vec{r}_k\}=\{r_{kx}\}$, and $3N$ momenta $p = \{\vec{p}_k\}=\{p_{kx}\}$,
    %but there is only $6(N-1)$ degrees of freedom when excluding the fixed frame, 
    %so phase space is really only $6(N-1)$-dimensional -- with $6(N-1)$ generalised coordinates.
}
\begin{equation}
...
%Here, the 
%Here, we will follow Newton, using an integration 
...
%The number of microstates corresponding to a given macrostate $n$ is called its multiplicity $\Omega(n)$.
...
%\end{equation}
%with the summation is to be carried out over all accessible states of the system (phase space), and
...
%The total multiplicity of all the macrostates is the number of microstates.

%
 %As an example, a particle can be a system, and an ensemble a set of N particles with a large N.
%In statistical mechanics, We imagine having a large number of copies of the \emph{same} system. 
%
%A statistical ensemble is a theoretical tool used to analyse a macroscopic system. 
%One imagines many copies (sometimes infinitely many) of the system. 
%\url{http://en.wikipedia.org/wiki/Statistical_mechanics#Statistical_ensembles}

%A statistical ensemble is determined by the probability for a certain microstate to occur.
%The probability that any given system in an ensemble will be in an energy eigenstate $i$ at temperature $T$ is 
...
%Stated in another manner, ergodicity requires the full constant-Hamiltonian surface to be accessible.
%Interestingly, it also requires some mixing, since a perfectly harmonic system will only sample a limited region.\cite[47-53]{Haile:1992}

%%of phase space that can be sampled,
%%as the probability of an atom passing an energy barrier decreases as [\ldots]. 

%For the study of phosphorus diffusion, non-ergodicity is not expected to be a problem, however. 
%The effect of mixing can actually be seen in fig \ref{fig:simple-vibration}, where the motion would have been perfectly harmonic if no mixing took place.
%The barriers to diffusion are also expected to be \emph{accessible}, although long simulations may be needed to actually cross them.
...
\end{equation}
From its second and fourth moments, % Frenkel, p. 126-127

% Simplify[Integrate[4 \[Pi] ((b m)/(2 \[Pi]))^(3/2) x^4 Exp[-b m x^2/2], {x, 0, \[Infinity]}, Assumptions -> {b > 0, m > 0}]]
\begin{equation}
  \langle v_k^2 \rangle = \int_0^{\infty} v_k^2 \rho_{\text{MB}}(v_k) \,dv_k = \frac{3}{m_k\beta},
\end{equation}
and
%Simplify[Integrate[4 \[Pi] ((b m)/(2 \[Pi]))^(3/2) x^6 Exp[-b m x^2/2], {x, 0, \[Infinity]}, Assumptions -> {b > 0, m > 0}]]
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\subsection{The Einstein-Smoluchowski diffusion relation}\label{sec:einstein-relation}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%\begin{equation}
%  D_{\alpha} = \frac{1}{3N_{\alpha}} \sum_{k=1}^{N_{\alpha}} \int_0^{\infty} \langle \vec{v}_k(t) \cdot \vec{v}_k(0) \rangle dt
%\end{equation}
...
%One way is to monitore the mean square displacement, the simulation length may be considered sufficient once a good fit for the slope to a straight line is obtained for use with \eqref{eq:einstein-relation}.
...
%The method generally suffers from severe drift, and is not recommended for MD.\cite[65]{Frenkel:1996}
...
A common approach, which is also used in {\vasp}, % function RANG in random.f90, called by subroutine INITIO in dyna.f90
...
%, and adding the coulombic interactions between the nuclei.\footnote{\comment{The Madelung energy found from an Ewald summation?}}
...
\begin{figure}[htbp]
  \centering
  \includegraphics{../figures/tests/thermostats/nothermo_simple}
  \caption{
...
\begin{figure}[htbp]
  ...
  \subfloat[Velocity-scaling]{
    \includegraphics{../figures/tests/thermostats/velocity-scaling}
  ...
  \subfloat[Nose-Hoover thermostat]{
    ...
    \includegraphics{../figures/tests/thermostats/nose-0-25}
    %\crossbox{7cm}
...
%Nose-type thermostats may under some circunstances, have problems with long-time energy conservation,
%according to earlier experience in my group.
%Energy can be transferred from the system into the nose system
%I'm not sure if this was due to the implementation of the algorithm in {\vasp} or a general problem.
%I suspect the former, since I did not find much information about this in literature.
%Nevertheless, a velocity-scaling thermostat was chosen 
...
%    The experimental
%Or using the average atom weight 28~u, we find $\frac{\SI{0.3}{\electronvolt}}{28\si{\amu}}\times \frac{\SI{1.60e-22}{\kilo\joule\per\electronvolt}}{\SI{1.66e-27}{\kilogram\per\amu}} \approx \SI{1030}{\kilo\joule\per\kilo\gram}$.

%The experimental heat of fusion is about 1925 kJ/kG according to \url{http://www.engineeringtoolbox.com/fusion-heat-metals-d_1266.html}.} although I'm not sure to what degree it makes sense to compare the values at all.
...
%\begin{equation}
%  \langle v_{\alpha}^2 \rangle = \frac{k_B}{m} T, 
%\end{equation}
...
%, lending notation from chemistry, while hopefully not giving associations to a giant molecule.
...
the ``classic'' bond-centred site (with $C_{3v}$ symmetry) and the \spl{110} site.
Special interest was taken in the \spl{110} site, since it has been identified as the most stable site 
...
% vp = IterativeVasprunParser('vasprun.1.xml')
% t = vp.get_trajectory()
% np.max(q.forces[-1])
...
Energetically, the tetrahedral site is less favourable than the hexagonal and \spl{110} sites by about 1~eV.
...
The figures \ref{fig:split110-2} and \ref{fig:split110} show the two lowest-energy variants of the \spl{110} configuration,
...
a \spl{110} variant, and which I will refer to as just the ``triangle'' configuration.
A relaxed \spl{110} configuration is already shifted in the [110] direction with respect to the shared lattice site,
...
while the NEB study by Liu\cite{Liu:2003} identified a \spl{110} configuration as minimum.
...
\begin{figure}[htbp]
  ...
  %    -348.846128 eV
  \subfloat[Tetrahedral, \SI{-1.188}{\electronvolt}]{
  ...
  %    -349.794169 eV
  % relaxed -> hexagonal
  \subfloat[Hexagonal, \SI{-2.136}{\electronvolt}]{
  ...
  %    -348.540818 eV
  \subfloat[Bond-centred $\langle111\rangle$, \SI{-0.883}{\electronvolt}]{
    \includegraphics[width=4.3cm]{../figures/initial_structures/bond-centered-111.png}
    \label{fig:bc111}
  }\\
  %
  \subfloat[split $\langle110\rangle$, \SI{-2.214}{\electronvolt} ]{
  ...
  %
  \subfloat[split $\langle110\rangle$, \SI{-2.215}{\electronvolt}]{
    %\includegraphics[width=4.3cm]{../notur/hipersol/si-pi/si64/test_structures/split-110-from-md/vmd_detail2.png}
    %\includegraphics[width=4.3cm]{../notur/hipersol/si-pi/si64/test_structures/split-110-from-md/stdrelax-short/vmd_detail2.png}
  ...
  %    -350.041718 eV
  \subfloat[``triangle'', \SI{-2.384}{\electronvolt}]{
    %\includegraphics[width=4.3cm]{../notur/hipersol/si-pi/si64/test_structures/split-110-from-md/vmd_detail2.png}
    %\includegraphics[width=4.3cm]{../notur/hipersol/si-pi/si64/test_structures/split-110-from-md/stdrelax-short/vmd_detail2.png}
...
Note that simulations were started either from a \spl{110} configuration (fig. \ref{fig:split110}) 
...
it quickly moves into a \spl{110}-like structure, as exemplified in fig. \ref{fig:cartoon2.1}~$\to$~\ref{fig:cartoon2.2},
and we could expect the same to happen for the triangle configuration, but it is a bit unfortunate that it was not tested.
%We can assume the same to happen in a simulation starting in a triangle configuration, but it's a bit unfortunate 0
...
\begin{figure}[htbp]
  ...
  \subfloat[P at site 2. A Si-Si pair shares the adjacent site in the plane above (not depicted).]{
  ...
  \caption{
    30 ps simulation at 1000 K. (c-g) shows 5 selected configurations viewed 
    ...
    Vibrations have beeen smoothed out by a 0.5~ps symmetric mean.
  }
  \label{fig:example1}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \subfloat[Relaxed initial structure, with P at hexagonal site.]{
...
Figure \ref{fig:example1} shows some snapshots and plots from a trajectory at 1000~K,
where phosphorus starts in a \spl{110} position, sharing the site labelled 2 with a Si atom.
During the first 18~ps, phosphorus moves between two split positions, 
centred about site 1 (fig. \ref{fig:cartoon1.2}) and 2 (fig. \ref{fig:cartoon1.1}), respectively,
and the intermediate position is similar to the ``triangle configuration''.
Such a movement is seen in many of the simulations, especially at low temperatures,
with Si-P-Si triangle-like structure moving in a quite correlated way back and forth between two lattice sites.
In this example, the mean Si-P-Si angle is about \ang{112}.
...
It is possible that this is a bias arising from starting in the \spl{110}, 
...
%After about 20~ps, P succeeds in obtains lattice site 1 for itself, pushing the Si atom into forming a Si-Si pair, 
%A little later, however, at about 25~ps, phosphorus makes a diffusive jump to the lattice site we call 4. 
...
but immediately (in less than 200~fs) enters a \spl{110}-like position (fig. \ref{fig:cartoon2.2}).
...
After 5~ps, the P atom makes a very short visit to lattice site 1 before returning to a rotated \spl{110}-like configuration.
...
%with many different processes often going on at the same time,
but I have included a snapshot showing a \spl{110}-like configuration (fig. \ref{fig:cartoon2.4}), which is seen all the time, 
...
%Since j trajectories $\vec{r}(t)$ are the 
%Since trajectories are not so often depicted, even in books on molecular dynamics, I've chosen to include two pictures showing what the atomic motion looks like.
...
\begin{figure}[htbp]
    \begin{center}
      ...
      %}
      \subfloat[Liquid state at \SI{1600}{\kelvin} (kinetic energy 204 meV/atom)]{
        \label{fig:dancea}
        \includegraphics[width=6cm]{../notur/hipersol/si-pi/si64/split110_1600/dance_1200x1200.png}
      }
        \subfloat[Crystalline state at \SI{1700}{\kelvin} (kinetic energy 216 meV/atom)]{
        \label{fig:danceb}
...
%Figure \ref{fig:danceb} shows an example of how the crystalline state may look like in a molecular dynamics simulation at elevated temperature.
%This is actually a metastable state well above the melting point of the system, 
%but while the microscopic motion is chaotic, the lattice, as indicated in the figure by the grid, is still well defined (the translational order parameter \eqref{eq:lattice_order_parameter} is > 0.8 using $\vec{G}_{111}$).
%Compare with figure \ref{fig:dancec}, showing the same system at a temperature 100 K lower, but here no lattice can be defined, and the system is clearly melted (lattice order parameter $\sim$ 0.1).
...
% Trajectories shown:
% Solid: split110_700: 10 ps extracted
% Solid: hexagonal_1700: 10 ps extracted
% Liquid: hexagonal_1700_2: 10 ps extracted
% 
\begin{figure}[htbp]
  \centering
  \includegraphics[width=12cm]{figs/rdfmelt}
...
%Alder, B. J  and W. G Hoover, ``Numerical Statistical Mechanics'' in ''Physics of Simple Liquids, H. N. V. Temperley, J. S. Rowlinson,
%and G. S. Rushbrooke, Eds., North-Holland, Amsterdam, 1968, Chapter 4
...
%Instead of an abrupt cutoff, other methods have been, such as evaluating the peak as symmetric, integrating to the top of the peak only, and multiplying the value by two, (VAR DET i Alder:1968?)
%but I'm not sure if there is really a physical motivation for such an approach.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=8cm]{figs/rdf_first_coord}
  \caption{Coordination numbers $n(r')$ from integrating $4\pi\rho g(r) r^2 $ for a Si$_64$P system
     ...
     % Note that minima and maxima, indicated by vertical lines, are evaluated from $g(r)$, not $g(r) r^2$,
     %which is why they appear to be shifted.  
...
%making for the calculation of partial distribution functions and 
%An interesting aspect of using molecular dynamics data, is that the 
%The radial pair distribution function can be obtained from experiment using x-ray diffraction, 
%but with molecular dynamics data we have 
%but 
%but an interesting aspect of molecular dynamics is that 
%it also provides the dynamic data underlying the function.
...
%since the 12 second nearest neighbours in the crystalline state are seen to be distributed quite evenly in the liquid state.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=10cm]{figs/tetrahedral_1600_neighboursdst_atom_all_nosmooth.png}
...
Temp (K) & $t$ (ps) & $\Delta r_{\text{P}}$ (Å) & $\langle\Delta r_{\text{Si}}\rangle$ (Å) & $D_{\text{P}}$ ($10^{-6}$~cm$^2$/s) & $D_{\text{Si}}$ ($10^{-6}$~cm$^2$/s) \\
    \midrule
         %700 & 29.0 & 1.9 & 0.3 & 2.01e-06 & 5.56e-08 \\
         %800 & 29.0 & 2.2 & 0.3 & 2.90e-06 & 7.68e-08 \\
         %900 & 27.5 & 1.1 & 0.3 & 7.71e-07 & 9.04e-08 \\
         %1000 & 29.0 & 4.2 & 0.4 & 1.02e-05 & 1.58e-07 \\
         %1100 & 29.0 & 1.9 & 0.4 & 2.18e-06 & 1.25e-07 \\
         %1200 & 29.0 & 2.5 & 0.5 & 3.65e-06 & 2.74e-07 \\
         %1300 & 29.0 & 1.2 & 0.5 & 7.90e-07 & 2.11e-07 \\
         %1300 & 89.0 & 10.8 & 0.5 & 2.17e-05 & 9.37e-08 \\
         %1400 & 59.0 & 3.8 & 1.2 & 4.17e-06 & 9.22e-07 \\
         %1400 & 50.6 & 2.2 & 0.8 & 1.64e-06 & 5.52e-07 \\
         %1400 & 79.0 & 2.2 & 1.4 & 1.03e-06 & 8.19e-07 \\
         %1400 & 89.0 & 0.8 & 1.5 & 1.12e-07 & 9.05e-07 \\
         %1450 & 59.0 & 2.8 & 1.4 & 2.16e-06 & 1.33e-06 \\
         %1500 & 59.0 & 8.9 & 1.1 & 2.22e-05 & 9.19e-07 \\
         %1500 & 79.0 & 4.5 & 1.5 & 4.35e-06 & 1.10e-06 \\
         %1500 & 59.0 & 5.3 & 1.0 & 7.91e-06 & 6.94e-07 \\
         %1550 & 14.0 & 3.3 & 0.8 & 1.29e-05 & 1.72e-06 \\
         %1550 & 59.0 & 3.7 & 1.2 & 3.79e-06 & 9.36e-07 \\
         %1600 & 8.1 (melted) & 5.3 & 1.9 & 5.75e-05 & 8.71e-06 \\
         %1600 & 27.9 (melted) & 4.2 & 2.5 & 1.07e-05 & 5.59e-06 \\
         %1600 & 19.0 & 1.2 & 0.5 & 1.34e-06 & 4.26e-07 \\
         %1600 & 27.4 (melted) & 3.8 & 2.3 & 8.68e-06 & 4.46e-06 \\
         %1650 & 7.5 (melted) & 4.5 & 2.0 & 4.50e-05 & 1.12e-05 \\
         %1700 & 5.2 (melted) & 2.6 & 1.8 & 2.21e-05 & 1.39e-05 \\
         %1700 & 18.8 & 2.1 & 0.9 & 3.89e-06 & 1.27e-06 \\
         ...
         % 700 & 30.0  &  2.4 &  0.1 & 3.32e-06 & 2.19e-08 \\
         % 800 & 30.0  &  2.1 &  0.1 & 2.56e-06 & 4.70e-08 \\
         % 900 & 28.5  &  1.2 &  0.1 & 7.75e-07 & 1.94e-08 \\
         %1000 & 30.0  &  4.6 &  0.2 & 1.15e-05 & 7.12e-08 \\
         %1100 & 30.0  &  1.0 &  0.1 & 5.61e-07 & 3.16e-08 \\
         %1200 & 30.0  &  1.3 &  0.1 & 9.46e-07 & 4.84e-08 \\
         %1300 & 30.0  &  1.8 &  0.2 & 1.78e-06 & 8.77e-08 \\
         %1300 & 90.0  & 10.5 &  0.3 & 2.04e-05 & 3.63e-08 \\
         %1400 & 60.0  &  2.9 &  1.1 & 2.34e-06 & 8.95e-07 \\
         %1400 & 51.6  &  3.4 &  0.5 & 3.78e-06 & 4.61e-07 \\
         %1400 & 80.0  &  3.6 &  1.3 & 2.71e-06 & 7.21e-07 \\
         %1400 & 90.0  &  1.9 &  1.3 & 6.98e-07 & 8.10e-07 \\
         %1450 & 60.0  &  4.3 &  1.2 & 5.20e-06 & 1.21e-06 \\
         %1500 & 60.0  &  8.3 &  0.8 & 1.91e-05 & 7.91e-07 \\
         %1500 & 80.0  &  5.1 &  1.3 & 5.38e-06 & 1.03e-06 \\
         %1500 & 60.0  &  6.6 &  0.8 & 1.19e-05 & 6.27e-07 \\
         %1550 & 15.0  &  2.3 &  0.4 & 5.64e-06 & 1.06e-06 \\
         %1550 & 60.0  &  4.3 &  1.0 & 5.23e-06 & 7.73e-07 \\
         %1600 &  9.1* &  5.1 &  1.4 & 4.66e-05 & 5.15e-06 \\
         %1600 & 28.9* &  7.1 &  1.9 & 2.94e-05 & 3.63e-06 \\
         %1600 & 20.0  &  2.2 &  0.2 & 3.93e-06 & 2.42e-07 \\
         %1600 & 28.4* &  4.6 &  1.8 & 1.26e-05 & 3.02e-06 \\
         %1650 &  8.5* &  3.3 &  1.4 & 2.16e-05 & 5.50e-06 \\
         %1700 &  6.2* &  1.7 &  1.2 & 7.61e-06 & 6.27e-06 \\
         %1700 & 19.8  &  1.3 &  0.6 & 1.32e-06 & 1.03e-06 \\
...
\begin{figure}[htbp]
  \centering
    \subfloat[Phosphorus (Pearson's correlation coefficient is 0.71)]{
      \label{fig:r2p}
      \includegraphics[width=6cm]{figs/hexagonal_1450_r2_p}
    }
    \subfloat[Silicon (Pearson's correlation coefficient is 0.97)]{
      \label{fig:r2si}
      \includegraphics[width=6cm]{figs/hexagonal_1450_r2_si}
    }
...
\begin{figure}[htp]
  \centering
  \includegraphics[width=8cm]{figs/arrhenius}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=8cm]{../notur/hipersol/si-pi/si64/hexagonal_1600/hexagonal_1600_dist_from_lattice.pdf}
  \caption{
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=8cm]{figs/time_at_lattice.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=8cm]{figs/firstsite_log.pdf}
...
%An attempt was made at quantifying the continuous trajectory into discrete jump, as introduced in .
%In order to do this, we must define what is meant by a lattice site, and what is meant by a jump.
%From molecular dynamics data, this is not necessarily obvious.

%With the phosphorus displacements listed in the previous section, it could also be interesting to the number of jumps from the data.
...
%averages over several shorter simulations with equal initial structures is likely
%to be a better way to study individual mechanisms.
%Rather than time-averages over a few, long simulations, 


%perhaps combined with automatic post-filtering of only the mechanism of interest.

%It is that similar analyses, with carefully controlled initial conditions, averaged over many trajectories

%is generally more successful than an analysis involving 

%approaches like the analysis of the time $\tau$ it took for the P atom to move from an interstitial site to a substitutional site

%Time τ in ps before the P atom enters a lattice site.

%many different mechanisms at play during the simulations in 

%but not enough to estimate migration energy
%hard with dopants due to low conc. simulating 65 atoms, but only using data from 1
%way to go forward to be to use the data to fit an empirical potential
%also difficult with many different mechanisms into play
%in some sense one of the more successful approach was the estimation of the activatino energy for int->si jump.
%Such study could be carried out in more well-defined manner: same initial structures,
%trying to distinguish different mechanisms
%generated data to kmd

%Determining the overall migration energy of phosphorus was not successful
%Estimating diffusion of dopants using \texit{ab intio} molecular dynamics 
...
%    $E$     & Total energy, $E = T + V$ \\
%    $T$     & Kinetic energy \\
%    $V$     & Potential energy \\ % for the nuclei, $E_v = E_e + E_{\text{mad}}$. \\
...
%    $\mathcal{T}(t)$  & Instantaneous temperature \\
...
%While {\vmd} can also read \texttt{vasprun.xml} files, it can be useful 
%it can be quite useful to be able to 
%It can unwrap motion across periodic boundaries

%The vtf writer writes both positions and forces, and the latter can be read by 

%\begin{figure}[htbp]
%  \centering
%  \includegraphics[width=6cm]{../notur/hipersol/si-pi/si64/split110_1000/vmdscene4.png}
%  \caption{Force visualisation}
%  \label{fig:forceviz}
%\end{figure}
