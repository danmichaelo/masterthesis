...
Diffusion\index{diffusion} is the redistribution of atoms\footnote{I will only be concerned with atoms here although the theories of diffusion are not limited to atoms} due to their thermal motion. 

Such redistribution occurs in all materials, but at a negligible rate at temperatures much below the melting point of the material.
...
As is not uncommon in physics, different descriptions exists depending on the scale of interest. 
On a macroscopic scale\footnote{\comment{Clarify macroscopic / microscopic?}
}, a description involving every single atom becomes impractical, and we are better off with a viscous fluid description.
Such a description was first introduced by Adolf Fick in 1855\cite{Fick:1855} as a phenomenological description involving empirical diffusion coefficients. 
...
\begin{equation}
  ...
  \label{eq:ficks1st}
\end{equation}
This relationship between the concentration gradient and the flux is known as Fick's first law, and the constant of proportionality $D$ is known as the coefficient of diffusion or \emph{diffusivity}\index{diffusivity} and are often reported in \si{\metre\squared\per\second} or \si{\centi\metre\squared\per\second}.
...
\section{Atomistic diffusion models}

\ldots

\section{The case of phosphorus in silicon}
